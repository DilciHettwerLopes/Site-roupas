...
if(is_numeric($id) && !empty($id) && $_GET['acao']=="apagar"){
    $busca = mysqli_query($link,'SELECT arquivo FROM feminino WHERE id='.$id);
    $busca = mysqli_fetch_object($busca);
    if(unlink('../img/produtos/'.$busca->arquivo)) {
        mysqli_query($link, "DELETE FROM feminino WHERE id=$id");
    ...
    header('Location:index.php?pagina=feminino/listagem');
}

if($pagina=='feminino/acoes' && $_POST) {
    extract($_POST);
    if(empty($nome) || empty($descricao) || empty($tamanho) || empty($preco)){
        ...
        if (!empty($id)) {
            header('Location:index.php?pagina=feminino/formulario&id='.$id);
        }else{
            header('Location:index.php?pagina=feminino/formulario');
        }
    }else {
        ...
        if (!empty($id)) {
            ...
            $update = mysqli_query($link, "UPDATE feminino SET arquivo='$arquivoDB',nome='$nome',descricao='$descricao',tamanho='$tamanho',preco='$preco' WHERE id=$id");
            if (mysqli_affected_rows($link) > 0) {
