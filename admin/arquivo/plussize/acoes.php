...
if(is_numeric($id) && !empty($id) && $_GET['acao']=="apagar"){
    $busca = mysqli_query($link,'SELECT arquivo FROM plussize WHERE id='.$id);
    $busca = mysqli_fetch_object($busca);
    if(unlink('../img/produtos/'.$busca->arquivo)) {
        mysqli_query($link, "DELETE FROM plussize WHERE id=$id");
    ...
    header('Location:index.php?pagina=plussize/listagem');
}

if($pagina=='plussize/acoes' && $_POST) {
    extract($_POST);
    if(empty($nome) || empty($descricao) || empty($tamanho) || empty($preco)){
        ...
        if (!empty($id)) {
            header('Location:index.php?pagina=plussize/formulario&id='.$id);
        }else{
            header('Location:index.php?pagina=plussize/formulario');
        }
    }else {
        ...
        if (!empty($id)) {
            ...
            $update = mysqli_query($link, "UPDATE plussize SET arquivo='$arquivoDB',nome='$nome',descricao='$descricao',tamanho='$tamanho',preco='$preco' WHERE id=$id");
            if (mysqli_affected_rows($link) > 0) {
            ...
            if(!empty($nomeArquivo)) {
                if (move_uploaded_file($arquivo['tmp_name'], '../img/produtos/' . $nomeArquivo)) {
                    $insert = mysqli_query($link, "INSERT INTO plussize VALUES (null,'$nome','$descricao','$tamanho', '$preco', '$nomeArquivo')");
                    if (mysqli_affected_rows($link) > 0) {
